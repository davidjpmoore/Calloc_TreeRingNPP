...
ggplot(data = valles.tot.dev) + facet_grid(SiteID~.) +
  ...
  scale_fill_manual(name="Uncertainty", values=c("green3", "blue", "red", "orange2"), labels=c("Total Uncertainty")) +
...
ggplot(valles.ind.dev[valles.ind.dev$Year >= 1925 & valles.ind.dev$Year <=2011,]) + facet_grid(SiteID ~ .) +
  ...
  #1) Increment Uncertainty
  geom_ribbon(aes(x=Year, ymin=Base - LB.inc, ymax= Base + UB.inc, fill="1"), alpha=0.6) +

  #2) Allometric Uncertainty -- separate for upper & lower to make things clearer
  geom_ribbon(aes(x=Year, ymin=Base - LB.allom, ymax= Base - LB.inc, fill="2"), alpha=0.6) +
  geom_ribbon(aes(x=Year, ymin=Base + UB.allom, ymax=Base + UB.inc, fill="2"), alpha=0.6) +
  
  #3) Density Uncertainty -- separate for upper & lower to make things clearer
  geom_ribbon(aes(x=Year, ymin=Base - LB.dens, ymax=Base - LB.allom, fill="3"), alpha=0.6) +
  geom_ribbon(aes(x=Year, ymin=Base + UB.dens, ymax=Base + UB.allom, fill="3"), alpha=0.6) +
  
  #4) Mortality Uncertainty -- separate for upper & lower to make things clearer
  geom_ribbon(aes(x=Year, ymin=Base - LB.mort, ymax= Base - LB.dens, fill="4"), alpha=0.6) +
  geom_ribbon(aes(x=Year, ymin=Base + UB.mort, ymax=Base + UB.dens, fill="4"), alpha=0.6) +

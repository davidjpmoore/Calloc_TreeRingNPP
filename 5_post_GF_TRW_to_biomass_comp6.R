...
library(car)
load("processed_data/allometries/allometries_6.Rdata")
allom.6 <- allom.temp 
allometries <- allom.6
...
save(bm.array, file="processed_data/Biomass_Array_Tree_kgm-2_component6.RData")
...
bm.tree <- merge(bm.tree, trees.use[,c("TreeID","Site","PlotID", "plot", "Canopy.Class", "DBH..cm.", "Live.Dead", "Distance", "Azimuth","Dated", "pft")], all.x=T, all.y=F)
summary(bm.tree)

write.csv(bm.tree, "processed_data/Biomass_Tree_kgm-2_component6.csv", row.names=F)
# ---------------------------------
...
write.csv(bm.plot2, "processed_data/Biomass_Plot_Total_kgm-2_component6.csv", row.names=F)
# ---------------------------------
...
# Getting the bm.pft into a friendly format
...
ggplot(bm.pft2[,]) + facet_grid(Site..Tower. ~ PlotID, scales="free") +
...
write.csv(bm.pft2, "processed_data/Biomass_Plot_pft_kgm-2_comp6.csv", row.names=F)
# ---------------------------------
...
# ---------------------------------
bm.site.pft2$n.species <- ifelse(bm.site.pft2$BM.Mean>0,1,0)
summary(bm.site.pft2)
...
bm.site3 <- aggregate(bm.site.pft2[,"n.species"], by=list(bm.site.pft2$SiteID, bm.site.pft2$Site, bm.site.pft2$Year), FUN=sum)
...
write.csv(bm.site2, "processed_data/Biomass_Site_Total_kgm-2_comp6.csv", row.names=F)

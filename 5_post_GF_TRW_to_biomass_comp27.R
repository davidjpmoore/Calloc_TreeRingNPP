...
library(car)
load("processed_data/allometries/allometries_27.Rdata")
allom.27 <- allom.temp
allometries <- allom.27
...
for(j in unique(trees.use$pft)){
  ...
  pft.use <- paste(unique(trees.use[trees.use$pft==j, "pft"])[1]) # If we don't have species-level, use a broad PFT
...
save(bm.array, file="processed_data/Biomass_Array_Tree_kgm-2_component1.RData")
load("processed_data/Biomass_Array_Tree_kgm-2_component1.RData")
...
bm.tree <- merge(bm.tree, trees.use[,c("TreeID","Site","PlotID", "plot", "Species", "Canopy.Class", "DBH..cm.", "Live.Dead", "Distance", "Azimuth","Dated", "pft")], all.x=T, all.y=F)
summary(bm.tree)

write.csv(bm.tree, "processed_data/Biomass_Tree_kgm-2_component1.csv", row.names=F)
# ---------------------------------
...
write.csv(bm.plot2, "processed_data/Biomass_Plot_Total_kgm-2_component1.csv", row.names=F)
# ---------------------------------
...
write.csv(bm.site.pft2, "processed_data/Biomass_Site_Species_kgm-2_comp1.csv", row.names=F)
...
write.csv(bm.site2, "processed_data/Biomass_Site_Total_kgm-2_comp1.csv", row.names=F)

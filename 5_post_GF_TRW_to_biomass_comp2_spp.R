...
save(bm.array, file="processed_data/Biomass_Array_Tree_kgm-2_component2_spp.RData")
load("processed_data/Biomass_Array_Tree_kgm-2_component2_spp.RData")
# ---------------------------------
...
for(s in 1:length(spp)){
  for(p in 1:length(plots)){ # will go by plot
    cols <- which(dimnames(bm.mean)[[2]] %in% trees.use[trees.use$PlotID==plots[p] & trees.use$Species==spp[s], "TreeID"])
	if(length(cols) > 0){
      if(substr(plots[p],1,1)=="V" | substr(plots[p],1,1)=="N"){ 
	    if(length(cols) > 1) {
